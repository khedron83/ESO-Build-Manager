...
class _SkillCard(QFrame):
    def __init__(self, slot_label: str, skill_name: str = "", accent: str = "#4a9eff",
        ...
        self.setFixedHeight(122 if show_icon else 56)
        ...
        if show_icon and filled:
            url = skill_icon_url(skill_name.strip())
            if url:
...
class BuildSheetWidget(QWidget):
    ...
    def _rebuild(self, build, skills, gear) -> None:
        ...
        self._vbox.addWidget(self._skills_section(build, skills))
        ...
        self._vbox.addWidget(self._gear_section(gear))
    ...
    def _skills_section(self, build, skills) -> QWidget:
        ...
        bars = [
            (0, "Front Bar", "#4a9eff", "#3a82e0"),
            (1, "Back  Bar", "#f97316", "#e06010"),
        ]
        for bar_idx, bar_label, accent, ult_accent in bars:
            row = QHBoxLayout()
            row.setSpacing(6)

            bar_lbl = QLabel(bar_label)
            bar_lbl.setFixedWidth(72)
            bar_lbl.setStyleSheet(
                f"color: {accent}; font-weight: bold; font-size: 11px; letter-spacing: 1px;"
            )
    ...
    def _cp_section(self, slots: list[str]) -> QWidget:
        ...
        grid = QGridLayout()
        grid.setSpacing(6)
        for row, (tree_name, color, offset) in enumerate(trees):
            ...
            for i in range(4):
                grid.addWidget(_SkillCard(str(i + 1), slots[offset + i], color), row, i + 1)
    ...
    def _stats_section(self, build) -> QWidget:
        ...
        vbox.setContentsMargins(0, 0, 0, 0)
        vbox.setSpacing(10)
        vbox.addWidget(_section_label("Stats & Food"))

        row = QHBoxLayout()
        row.setSpacing(32)

        for label, value, color in [
            ("Health",   build.attribute_health,   "#f87171"),
            ("Magicka",  build.attribute_magicka,  "#60a5fa"),
            ("Stamina",  build.attribute_stamina,   "#4ade80"),
        ]:
            cell = QWidget()
            ...
            cell_l.setSpacing(2)
            lbl = QLabel(label)
            lbl.setStyleSheet("color: palette(windowText); font-size: 10px; font-weight: bold; letter-spacing: 1px;")
        ...
        for label_text, value in [("Food / Drink", build.food_buff), ("Mundus Stone", build.mundus_stone)]:
