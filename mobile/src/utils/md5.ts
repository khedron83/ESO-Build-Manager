// Minimal MD5 for UESP image URL computation — no deps needed
...
function md5(str: string): string {
  ...
  const len32 = (len8 + 8 >> 6) + 1;
  const x: number[] = new Array(len32 * 16).fill(0);
  for (let i = 0; i < len8; i++) x[i >> 2] |= bytes[i] << ((i & 3) << 3);
  ...
  return [a, b, c, d]
    .map(n => (n < 0 ? n + 0x100000000 : n).toString(16).padStart(8, '0')
      .replace(/../g, m => m[1] + m[0]))  // little-endian byte swap per pair
    .join('');
}

export function uespUrl(filename: string): string {
  const fname = filename.replace(/ /g, '_');
  const hash = md5(fname);
  return `https://images.uesp.net/${hash[0]}/${hash.substring(0, 2)}/${fname}`;
}
